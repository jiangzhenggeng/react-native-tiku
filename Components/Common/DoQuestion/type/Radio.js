            ...
            <ScrollView ref="ScrollView" showsVerticalScrollIndicator={false} style={styles.container}>
                ...
                <View style={{paddingTop:5,paddingBottom:10}}>
                    <View style={{paddingLeft:16,paddingRight:16}}>
                        <Text style={styles.anserText}>{String(question.question).replace(/<[^>]+>/ig,'').replace(/&nbsp;/ig,' ').replace(/&[a-z]{2,6};/ig,'')}</Text>
                    </View>
    ...
    showOptionList(option){
        return (
            <RadioGroup
                ...
                ref={o=>this._RadioGroup=o}
                disabled={this.disabled}
            >
                {option.map((item,i)=>{
                    return (
                        <RadioButton style={{paddingLeft:16,paddingRight:16}} key={i} value={item.optionname} label={item.optionname}>
